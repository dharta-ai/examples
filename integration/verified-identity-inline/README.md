...
vouches for your signed-in user.** It's also exactly how dharta.sh's own home-page
...
| **Dharta provides** | a verified `sub`, a per-user durable workspace, metering |

## How it works

```
 your page                 your backend                     Dharta
 ─────────                 ────────────                     ─────
 load page  ──fetch──▶  /api/dharta-identity
...
Dharta recomputes the HMAC at mint time and trusts the `sub` **only** on a match —
...
1. **Have an agent on Dharta** (e.g. follow [`../../apps/claude-code/hello-world`](../../apps/claude-code/hello-world)),
...
3. **Wire it up:**
   ...
   export DHARTA_IDENTITY_SECRET=<the secret you just generated>
...
- [`server.js`](server.js) — the minimal backend: the `/api/dharta-identity` mint + a static file server. Node built-ins only, no dependencies.
