...
perform bounded actions, and leave durable work behind. Dharta runs the Agent
App. Cinderlane owns the CRM interface, domain services, database, integrations,
and product billing.

Dharta does not become the CRM system of record. The Cinderlane API does not
become an agent harness. The MCP server is the typed bridge between them.

## Components

`apps/web` renders the CRM and embeds the Dharta agent. It requests a short-lived
Dharta identity assertion from the API after the normal Cinderlane login.

`apps/api` owns tenant membership, authorization, domain writes, identity
assertions, audit records, and links between CRM records and Dharta Artifacts.
...
`apps/agent` is the deployable Agent App. Dharta combines its harness,
instructions, skills, tools, and Artifact Types into immutable releases. Each
dharta is a durable instance with a persistent workspace.
...
| Agent drafts and research | Dharta workspace | Durable per-instance working state |
| Produced reports and plans | Dharta Artifact versions | User-visible work product |
| Artifact links on accounts | CRM database | Product navigation and retention |
| Tool and mutation audit | CRM audit log plus Dharta telemetry | Cross-boundary accountability |
...
The Agent App receives no database credential. The MCP server receives no Dharta
