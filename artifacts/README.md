...
These Agent Apps each produce exactly one Dharta Artifact. They are ordered by
...
| 03 | [`data-table`](03-data-table/) | CSV-backed table | Data rendered with a Dharta-owned file viewer. |
...
## Expected Dharta experience

When an agent produces an Artifact, Dharta adds an **Artifact ready** card to the
conversation and adds the current version to the persistent **Artifacts** list.
Dharta does not automatically open the Artifact and interrupt the conversation.
